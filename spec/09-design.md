...
## Sample exchange block (landing, section 2 of `05-landing.md`)

The demo section needs to read as a real conversation without imitating
WhatsApp's brand. Two stacked bubbles on `--calcaire-deep`: the question in a
lighter panel aligned left with a `--sable` hairline, the answer aligned left
too (not right — this isn't a chat UI, it's a quoted exchange) in
`--calcaire` with a 2px `--mediterranee` left rule, body type, generous line
height. No avatars, no timestamps, no green. The caption below sits in
`--sable` meta type. This is the second-most prominent element on the page
after the H1 — give it real vertical space.
